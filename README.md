...
1. Run the project like we did previously including all of the deployment scrips in the `/deployment` folder
1. There are three new services installed in the project:

* Kafka
* LocationProducer (gRPC client which receives location data and sends them to a kafka queue)
* LocationConsumer (python client which reads the kafka message queue and stores them in the database)
...
# copy name of location-producer
